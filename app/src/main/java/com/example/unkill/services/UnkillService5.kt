...
class UnkillService5 : Service() {
    ...
    override fun onCreate() {
        super.onCreate()
        Log.d(TAG, "UnkillService5 created")
        createNotificationChannel()
    }

    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        Log.d(TAG, "UnkillService5 started")
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(
                ...
                description = "Unkill Service Instance 5"
                setShowBadge(false)
            }
    ...
    private fun createNotification(): Notification {
        return NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle("Unkill Service 5")
        ...
        scope.launch {
            while (isActive && isMonitoring) {
                try {
                    ...
                    delay(20000) // Monitor every 20 seconds (different timing)
    ...
    override fun onDestroy() {
        super.onDestroy()
        Log.d(TAG, "UnkillService5 destroyed")
        isMonitoring = false
        scope.cancel()
    }
}
