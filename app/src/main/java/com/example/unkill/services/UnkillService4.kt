...
class UnkillService4 : Service() {
    ...
    override fun onCreate() {
        super.onCreate()
        Log.d(TAG, "UnkillService4 created")
        createNotificationChannel()
    }

    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        Log.d(TAG, "UnkillService4 started")
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(
                ...
                description = "Unkill Service Instance 4"
                setShowBadge(false)
            }
    ...
    private fun createNotification(): Notification {
        return NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle("Unkill Service 4")
        ...
        scope.launch {
            while (isActive && isMonitoring) {
                try {
                    ...
                    delay(18000) // Monitor every 18 seconds (different timing)
    ...
    override fun onDestroy() {
        super.onDestroy()
        Log.d(TAG, "UnkillService4 destroyed")
        isMonitoring = false
        scope.cancel()
    }
}
